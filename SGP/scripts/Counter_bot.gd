...
func _ready():
	...
	Global.turn_over.connect(change_counter)
			...
			if card_name in VARIATIONS:
				...
				if card_name == "fruit":
					for chr in variation:
						...
						to_be_displayed.append([lbl, str(int(lbl.text) + 1)])
				else:
					# maki and uramaki rolls
					var lbl = self.get_node(card_name + "_counter") 
					to_be_displayed.append([lbl, str(int(lbl.text) + int(variation))])
			else:
